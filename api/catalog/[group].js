...
function stats(records) {
  ...
  const ages = records
    .map(r => (now - epochToDate(r.line1).getTime()) / 3600000)
...
async function fetchUpstream(group) {
  ...
  try {
    const url =
      `${UPSTREAM}?GROUP=${encodeURIComponent(group)}&FORMAT=tle`;
...
export default async function handler(req, res) {
  ...
  const due =
    !existing ||
    now - existing.fetchedAtMs >= REFRESH_MS;
